...
function App() {
  ...
  const handleThemeSelect = async (selectedTheme: string, selectedDifficulty: DifficultyLevel) => {
    ...
    try {
      ...
      setLearningList([]); // Reset learning list when starting fresh
  ...
  const handleStartCustomSession = async (inputString: string) => {
    ...
    try {
        ...
        setWords(results);
        setLearningList(results); // For custom sessions, we likely want to learn all immediately
  ...
  const handleWordsComplete = async () => {
    setIsLoading(true);
    setStage(AppStage.ARTICLE_GENERATION); // Show loading state
    try {
      const wordList = words.map(w => w.word);
      const generatedArticle = await generateArticle(theme, wordList);
  ...
  const handleLoadSession = (session: LearningSession) => {
      ...
      setLearningList(session.words); // Set all words as ready to learn
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] text-gray-800 font-sans">
      <Header onReset={handleReset} onOpenNotebook={handleOpenNotebook} />
      ...
      <main className="container mx-auto px-4 py-8 pb-32">
           ...
           <div className="max-w-5xl mx-auto">
                 ...
                 <div className="mb-8 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-3">
                        <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider">
                            Ready to Learn ({learningList.length})
                        </h3>
                    </div>
             ...
             <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8 relative">
                ...
                <div className="flex flex-col items-center space-y-4 max-w-lg mx-auto" ref={bottomRef}>
                    ...
                    >
                        Start Learning ({learningList.length > 0 ? learningList.length : words.length} Words)
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {stage === AppStage.WORD_LEARNING && (
           <WordLearning words={words} onComplete={handleWordsComplete} theme={theme} />
        )}

        {stage === AppStage.ARTICLE_GENERATION && (
           <div className="flex flex-col items-center justify-center h-96">
              <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-primary mb-4"></div>
              <p className="text-xl font-medium text-gray-600">Generating a reading passage...</p>
           </div>
        )}
